...
if (type === 'admin' && user.role === 'admin') {
  ...
  if (passwordMatch) {
      req.session.user = { id: user.id, username: user.username, role: user.role ,status:"active"};
  ...
  } else if (type === 'staff' && user.role === 'staff') {
    const passwordMatch = await bcrypt.compare(password, hashedPassword);
    if (passwordMatch) {
      req.session.user = { id: user.id, username: user.username, role: type ,status:"active"};
      res.json({ success: true, user: req.session.user });
    } else {
      console.log("Error")
      res.status(401).json({ success: false, message: 'Invalid username or password' });
    ...
    res.status(401).json({ success: false, message: 'Invalid user type or credentials' });
  }
  } catch (err) {
...
// // Example usage
// (async () => {
//   const password = 'admin123';  // Replace with any password you want to hash
...
function isAdmin(req, res, next) {
  ...
  if (req.session.user && req.session.user.role) {
    ...
    return res.status(400).send('Bad Request: User role is not defined');
  }
}
...
app.get('/getNextStaffId', async (req, res) => {
  try {
    const request = new sql.Request(pool);
    // Fetch the last staff ID
    const result = await request.query('SELECT TOP 1 staffid FROM RegisterartionStaff ORDER BY staffId DESC');
    const lastId = result.recordset[0]?.staffid || 'S000'; // If no records, start from S000

    // Increment the ID
    const numericPart = parseInt(lastId.substring(1)); // Extract the numeric part
    const nextId = `S${String(numericPart + 1).padStart(3, '0')}`; // Format the next ID
...
app.get('/getNextDoctorId', async (req, res) => {
  try {
    const request = new sql.Request(pool);
    // Fetch the last staff ID
    const result = await request.query('SELECT TOP 1 doctorid FROM Doctor ORDER BY doctorid  DESC');
    const lastId = result.recordset[0]?.doctorid  || 'D000'; // If no records, start from S000

    // Increment the ID
    const numericPart = parseInt(lastId.substring(1)); // Extract the numeric part
    const nextId = `D${String(numericPart + 1).padStart(3, '0')}`; // Format the next ID
...
app.get('/getNextMedicineId', async (req, res) => {
  try {
    const request = new sql.Request(pool);
    // Fetch the last medicine ID
    const result = await request.query('SELECT TOP 1 medicineid FROM medicine ORDER BY medicineId DESC');
    const lastId = result.recordset[0]?.medicineid || 'M000'; // If no records, start from M000

    // Increment the ID
    const numericPart = parseInt(lastId.substring(1)); // Extract the numeric part
    const nextId = `M${String(numericPart + 1).padStart(3, '0')}`; // Format the next ID

    res.json({ success: true, nextId: nextId });
  } catch (err) {
    console.error('Error fetching next medicine ID:', err);
    res.status(500).json({ success: false, message: 'Database error' });
  }
});
...
app.post("/check-medicine-quantity", async (req, res) => {
  const { medicineid, quantity } = req.body;
  console.log(req.body);
  console.log("MedID: " + medicineid);

  try {
      // Create a new request object
      const request = new sql.Request(pool);

      // Fetch the available quantity of the medicine
      const fetchQuery = "SELECT quantity FROM medicine WHERE medicineid = @medicineid";
      request.input("medicineid", sql.NVarChar, medicineid); // Use sql.NVarChar for VARCHAR fields

      const result = await request.query(fetchQuery);

      if (result.recordset.length > 0) {
          const availableQuantity = result.recordset[0].quantity;

          if (quantity > availableQuantity) {
              // If requested quantity is greater than available
              return res.json({
                  success: false,
              ...
              const updateQuery = "UPDATE medicine SET quantity = quantity - @quantity WHERE medicineid = @medicineid";
              request.input("quantity", sql.Int, quantity); // No change needed for the integer quantity
...
app.get('/getNextServiceId', async (req, res) => {
  try {
    const request = new sql.Request(pool);
    // Fetch the last staff ID
    const result = await request.query('SELECT TOP 1 serviceid FROM Services ORDER BY serviceid  DESC');
    const lastId = result.recordset[0]?.serviceid  || 'SP00'; // If no records, start from S000

    // Increment the ID
    const numericPart = parseInt(lastId.substring(2)); // Extract the numeric part
    const nextId = `SP${String(numericPart + 1).padStart(2, '0')}`; // Format the next ID

    res.json({ success: true, nextId: nextId });
  } catch (err) {
    console.error('Error fetching next service ID:', err);
    res.status(500).json({ success: false, message: 'Database error' });
  }
});
...
app.get('/getNextWardId', async (req, res) => {
  try {
    const request = new sql.Request(pool);
    // Fetch the last staff ID
    const result = await request.query('SELECT TOP 1 wardno FROM Ward ORDER BY wardno  DESC');
    const lastId = result.recordset[0]?.wardno  || 'W000'; // If no records, start from S000

    // Increment the ID
    const numericPart = parseInt(lastId.substring(1)); // Extract the numeric part
    const nextId = `W${String(numericPart + 1).padStart(3, '0')}`; // Format the next ID

    res.json({ success: true, nextId: nextId });
  } catch (err) {
    console.error('Error fetching next ward ID:', err);
    res.status(500).json({ success: false, message: 'Database error' });
  }
});
...
app.get('/activeStaff', async (req, res) => {
  console.log('Route hit: /activeStaff');
  try {
      const result = await pool.query(`SELECT staff_id, fname, lname, designation FROM ActiveStaffMembers`);
      res.json(result.recordset);
  } catch (err) {
...
app.post('/add_inpatient', async (req, res) => {
  const {
    admissionno, fname, lname, cnic_no, age, gender, phone, doctorid, staffid, wardno,status, medicalInfo, serviceInfo
  } = req.body;
...
app.get('/getNextInpatientId', async (req, res) => {
  try {
    const request = new sql.Request(pool);
    // Fetch the last staff ID
    const result = await request.query('SELECT TOP 1 admissionno FROM Inpatient ORDER BY admissionno DESC');
    const lastId = result.recordset[0]?.admissionno  || 'IP00'; // If no records, start from S000

    // Increment the ID
    const numericPart = parseInt(lastId.substring(2)); // Extract the numeric part
    const nextId = `IP${String(numericPart + 1).padStart(2, '0')}`; // Format the next ID

    res.json({ success: true, nextId: nextId });
  } catch (err) {
    console.error('Error fetching next Inpatient ID:', err);
    res.status(500).json({ success: false, message: 'Database error' });
  }
});
app.get('/getNextOutpatientId', async (req, res) => {
  try {
    const request = new sql.Request(pool);
    // Fetch the last staff ID
    const result = await request.query('SELECT TOP 1 patientid FROM Outpatient ORDER BY patientid DESC');
    const lastId = result.recordset[0]?.patientid  || 'IP00'; // If no records, start from S000

    // Increment the ID
    const numericPart = parseInt(lastId.substring(2)); // Extract the numeric part
    const nextId = `OP${String(numericPart + 1).padStart(2, '0')}`; // Format the next ID

    res.json({ success: true, nextId: nextId });
  } catch (err) {
    console.error('Error fetching next Outpatient ID:', err);
    res.status(500).json({ success: false, message: 'Database error' });
  }
});
// to fetch wards
